...
def main(E_prop, angles, nu_type, cross_section_model, pn_model, idepth, ch_lepton, fac_nu, stats, prop_type, elep_mode, htc_mode, job_num):
    '''
    Parameters
    ----------
    E_prop : ndarray
    ...
    htc_mode : str
        Option to simulate in HTC mode. Can be yes or no.
    job_num : int
        Job number from cluster. Default is 0. 
    Returns
    -------
    NONE
    ...
    for energy in sorted(E_prop):
        ...
        for angle in sorted(angles):
            ...
            no_regen, regen = Run.run_stat_single(10**energy, angle, job_num, nu_xc, nu_ixc, depthE, dwater, xc_water, xc_rock, lep_ixc_water, lep_ixc_rock, alpha_water, alpha_rock, beta_water, beta_rock, xalong, cdalong, ithird, idepth, lepton_int, fac_nu, stats, prop_type_int)
            ...
            if htc_mode == 'no': # htc mode off
                ...
                with open("pexit_%.2f_%.1f_%d.dat" % (energy,angle,job_num), "w") as pexit_file:
                    pexit_file.write("%.5e\t%.5e\t%.5e\t%.5e\n" % (10**energy,angle,no_regen,regen))
                    
                    eout_name = "eout_{:.2f}_{:4.1f}_{:3d}.dat".format(energy, angle, job_num)
                    Pout_name = "Pout_{:.2f}_{:4.1f}_{:3d}.dat".format(energy, angle, job_num)
                    os.rename(eout_name, "eout_{:.2f}_{:.1f}_{:d}.dat".format(energy, angle, job_num))
                    os.rename(Pout_name, "Pout_{:.2f}_{:.1f}_{:d}.dat".format(energy, angle, job_num))
        ...
        if htc_mode == 'no': # htc mode off; do some post-processing
            ...
            Data.add_cdf(ch_lepton, energy, eout_list, out_file, htc_mode=False, arg=None) # adds the binned cdf values for all neutrino energies and angles in an output file, to the output file.
